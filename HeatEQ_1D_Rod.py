...
# Time-stepping loop
for n in range(M):  # Loop through time steps
    # Apply Dirichlet boundary conditions (fixed temperature at boundaries)
#    u[n+1, 0] = 20     # Left boundary fixed at 20°C
#    u[n+1, N] = 20     # EXPRESSION FOR RIGHT BOUNDARY CONDITION (FIXED 20°C)

    for i in range(1, N):
        u[n+1, i] = u[n, i] + r * (u[n, i+1] - 2*u[n, i] + u[n, i-1])   # expressions for update
    u[n+1, 0] = u[n+1, 1]
    u[n+1, N] = u[n+1, N-1]
